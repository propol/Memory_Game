...
/**
 * 
 * Implements the simple Multi Player mode of the game. A label signing the player's turn is set at the top of the frame.
 * If a player matches 2 cards, he plays again. If not, it's the next player's turn.
 * In the scenario where one or more of the players is AI, their turn is after turn 2.
 * For example, if there is one AI, it plays on "Player 2" turn, no matter the number of players.
 * If there are two AI, they play on "Player 2" and "Player 3" turn and so on.
 * 
 * @param t1 Thread used to create an AI object when one of the players is AI.
 * @param t2 Thread used to create another AI object when two of the players are AI.
 * @param t3 Thread used to create another AI object when three of the players are AI.
 * @param Input String used to get the number of players that user types in the first message dialog box that appears when this mode is on.
 * @param SecondInput String used to get the number of AI players the user indicates in the second message dialog box appeared, when this mode is on.
 * @param j Integer that holds the return of the Converter method.
 * @param label JLabel used to display the player's turn during the game.
 * @param panel JPanel where the label is put.
 * @param panel2 JPanel where the cards for the game are put.
 * 
 * The rest params have already been exlpained in previous classes therefore they are not explained here.
 */
public class Multi_Player extends JFrame
{
    Thread t1;
